...
import { WrongAnswerAttribution } from './WrongAnswerAttribution';
import { QuizSubmitSummary } from './QuizSubmitSummary';
...
export function QuizPanel({ wb, initialSource = 'fixed' }: Props) {
  ...
  /** 按 `itemId` 索引归因，渲染错题时 O(1) 取到 —— 数组顺序不保证与题目一致 */
  const attributionByItem = new Map(
    (report?.results ?? []).map((result) => [result.itemId, result.attribution]),
  );
  /*
   * ⚠️ 这里原有一个 `loaded` 状态（记住"这组题实际是用哪个主题/来源取回来的"，`I20②`）。
   *
   * `P-B9` 之后**它没有存在意义了**：上报只送 `itemId` + 选中的选项，
   * `topic`/`source` 一律由服务端按 `itemId` 从题库反查 —— 学生中途点一下
   * 切换按钮（题不会重取）也不会再让画像记录张冠李戴，因为那些字段根本不由前端提供。
   * `I20②` 描述的那一类缺陷由**接口形状**消除，而不是靠面板自己记准一个变量。
   */
    ...
    <section className="panel">
      ...
      {!items ? (
        ...
        <>
            ...
            <div key={item.id} className="quiz-item">
              ...
              {submitted && (
                <div className="quiz-explain">
                  ...
                  {/*
                    错题归因（`P-B9`）。只在**答错**时显示 —— 答对了本来就没有"错在哪"可说。
                    `attribution === null` 有两条路：报告未回（还在上传中/失败）与"推不出来"。
                    前者用 `report?.outcome === 'sent'` 区分开，**不把"还没拿到"写成"推不出来"**。
                  */}
                  {submitted && picked[item.id] !== item.answer && (
                    <WrongAnswerAttribution
                      attribution={attributionByItem.get(item.id) ?? null}
                      settled={report?.outcome === 'sent'}
                    />
                  )}
            ...
            {!submitted ? (
              <button
                ...
                onClick={() => {
                  setSubmitted(true);
                  /*
                   * 只上报"学生选了哪个选项"（`P-B9`）：判分与归因由服务端按 `itemId`
                   * 反查题库完成 —— 前端不再自报 total/correct，也就不会报错。
                   */
                  void wb
                    .reportQuizAttempt({
                      /* `flatMap` 而不是 `filter().map()`：后者收窄不了索引访问的类型 */
                      answers: items.flatMap((item) => {
                        const choice = picked[item.id];
                        return choice ? [{ itemId: item.id, selectedOptionId: choice }] : [];
                      }),
                    })
                    .then(setReport);
                }}
          ...
          {submitted && <QuizSubmitSummary report={report} correct={correct} total={items.length} />}
        </>
      )}
    </section>
  );
