      ...
      <aside className="shell-side">
        ...
        <details className="side-note">
          ...
          <div className="side-note-body">
            ...
            {/*
              * 📌 2026-09-24 代改（B 执行，待 A 复核）。
              *
              * 原文写的是「多智能体编排、错题归因」—— **这两项已于 2026-09-23 接入**
              * （`P-B8` 的 `degraded[]` 由答疑面板显示，`P-B9` 的归因由练习面板显示）。
              * 把它们继续列在「本页尚未接入」里，等于**在运行中的页面上说一句假话** ——
              * §8.4 要求「未实现的能力必须如实标注」，反过来**已实现的也不得标成未实现**，
              * 否则这份清单就不再可信（读者一旦发现一次不实，整份清单都失去意义）。
              *
              * 现清单只剩一项，且是**真的**：`GET /api/sessions` 接口可用，
              * 但前端**确实没有调用它**（`grep` 过 `api.ts` 与 `useWorkbench.ts`，0 处引用），
              * 左栏用的是 `sessionStorage` 里的本标签页记录。跨刷新/跨设备的恢复尚未定案。
              * ⇒ 「接口归属」与「页面有没有用上」是两件事，这条说的是后者。
              */}
            <div className="footer-block">
              <strong>本页尚未接入</strong>
              <span>
                服务端的历史会话列表（接口 GET /api/sessions 已可用，但左栏现在用的是本标签页内的
                会话记录 —— 跨刷新、跨设备的会话恢复尚未定案）。
                这里只列「接口有了、页面没用上」的项；入口要么不出现，要么出现时明确说明不可用，
                不以占位内容冒充已实现。
              </span>
            </div>
            <div className="footer-block">
