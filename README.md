...
**骨架可运行。** 说明书已更新至 **V2.0**（2026-09-17）；前后端骨架已搭建，回归脚本 137 项通过
（`store` 22 / `errors` 34 / `guards` 81）。

V2.0 相对 V1.4 的扩张尚未落地，**以下能力目前为零实现，不得视为可用**：

- **符号验证**（P0）—— 引擎选型已定（纯 TS / `mathjs`），代码未开始
- **图谱持久化**（P0）—— `/api/knowledge` 目前**算完即丢弃**；`GET /api/graph` 无数据可读
- **学习画像**（P0）—— 无 `POST /api/profile`
- **教师视图 / 语音 / 多智能体 / 图谱可视化 / 错题归因**（P1）—— 仅有设计，见
  `docs/plans/2026-09-17-第6轮-V2.0全量对齐.md` §6

接口形态为 **7 个**（V2.0 目标 **10 个**），且 `GET /api/quiz` 将变更为 `POST /api/quiz`；
契约迁移方案见 `docs/tech/2026-09-17-契约迁移-V2.0-十接口与六态.md`。

前端 A2–A8 尚为空白：`apps/web/src/App.tsx` 目前仅实现轻路径的文字提问。
