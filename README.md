...
- **在线链接已交付并已重部署**（`P-C14` / `I37`）：<https://weijifen-xueban-315516-12-1490356402.sh.run.tcloudbase.com/>
  ...
  **2026-09-21 20:34 已重部署到 `eb06ab6`（第 005 次发布，全量流量）**：符号验证、图谱边与
  **图片识别**三项在线上均已生效。访问口径与已知限制见下「已知限制」。
...
- **线上走真实模型通道，回归仍以 `mock` 为主**：线上（005 版）`modelProvider=deepseek`、`mock=false`；
