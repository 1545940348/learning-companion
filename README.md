...
回归脚本 **681 项全部通过**（2026-09-23 逐套件本机复现）：`verify:all` **301**（`store` 22 / `errors` 34 / `guards` **88** / `graph` 57 / **`symbolic` 69** / **`vision` 31**）、
`verify:flow` **48**（真实 HTTP）、`verify:dist` **11**（构建产物形态，含**运行形态自证**）、
`verify:render` **172**（前端渲染冒烟；含外壳、左栏与对话模型的 47 项）、**`verify:voice` 68**（语音输入专项：用**打桩的假识别引擎**覆盖九类分支
...
- **无法做浏览器视觉验证**：本环境不可安装 `agent-browser`，只能以 `verify:render`（125 项）做渲染冒烟，
