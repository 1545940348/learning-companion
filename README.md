...
回归脚本 **376 项全部通过**：`verify:all` **190**（`store` 22 / `errors` 34 / `guards` 81 / `graph` 53）、
`verify:flow` **36**（真实 HTTP）、`verify:dist` **11**（构建产物形态，含**运行形态自证**）、
`verify:render` **88**（前端渲染冒烟）、`verify:serve-web` **18**（同源托管：SPA 回退的**边界**、`/api` 不被吞）、
`verify:repo` **33**（敏感信息 / 依赖与许可证 / 提示词快照）。
...
| **在线链接 / 部署** | **尚未部署**（四件套中唯一未完成的一件），但**已无未知项**：载体已定（腾讯云 CloudBase 云托管，`D4` 2026-09-18 拍板），**同源托管本地已实现并验证**（`P-C13`，专项 `verify:serve-web` 18 项），只剩在控制台执行。⚠️ 用默认域名打开会先出现一次「访问提示中间页」，点「确定访问」后进入 | `P-C14` |
