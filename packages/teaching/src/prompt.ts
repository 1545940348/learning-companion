...
输出 JSON（除代码块外不要写多余文字）：
{
  "content": "200—400 字的补充说明，纯文本，不要 Markdown 标题、不要开场白与总结语",
  "claims": []
}

「claims」用于让系统**逐条验证**你写出的数学结论，使补充内容可以被程序核对。
规则：
- **只列 content 中真实出现过的确定结论**；没有具体结论就写空数组，不要硬凑
- 每条选一种格式：
  {"kind":"derivative","expr":"x^2","at":1,"claimed":2}
  {"kind":"tangent","expr":"x^2","at":1,"claimed":"2*x - 1"}
  {"kind":"monotonic","expr":"x^3-3*x","claimed":{"inc":[[-1000000,-1],[1,1000000]],"dec":[[-1,1]]}}
  {"kind":"extremum","expr":"x^3-3*x","claimed":[-1,1]}
- expr 与 claimed 中的式子用 mathjs 写法：乘号用 *，幂用 ^，**不要写 LaTeX**
- 开区间端点用大数表示（如 ±1000000）—— JSON 没有「无穷大」这个值

⚠️ **拿不准就不要写进 claims**。系统会把「没有 claims」的补充如实标为「未验证」，
这比写一条错断言要好得多。**宁可未验证，不可写错。**`;
